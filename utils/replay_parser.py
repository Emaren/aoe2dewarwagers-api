...
def _parse_match_fallback_bytes(replay_path, file_bytes, parse_error):
    ...
    try:
        ...
        logging.error(f"❌ parse_match DE fallback failed: {fallback_error}")
        return None

    players = []
    for raw_player in list(getattr(match, "players", []) or []):
        civilization_id = _normalize_rating(getattr(raw_player, "civilization_id", None))
        players.append(
            {
                "name": getattr(raw_player, "name", None) or "Unknown",
                "number": _normalize_rating(getattr(raw_player, "number", None)),
    ...
    stats = {
        ...
        "map": {"name": map_name or "Unknown", "size": "Unknown"},
        "game_type": str(
            getattr(match, "type", None)
            or getattr(match, "game_version", None)
            or "Unknown"
        ),
        ...
        "key_events": {
            ...
            "file_language": getattr(file_info, "language", None)
            if file_info is not None
            else None,
            "file_size": getattr(file_info, "size", None)
            if file_info is not None
            else None,
            "perspective_player_name": str(getattr(file_info, "perspective", ""))
            if file_info is not None
            else None,
        },
        "completed": completed,
        "disconnect_detected": False,
        "parse_reason": "de_live_parse_match_fallback",
        "played_on": dt.isoformat() if dt else None,
    }

    logging.warning(
        "⚠️ parse_match DE fallback used for %s after summary parse failed: %s",
        replay_path,
        parse_error,
    )
    return stats
...
def _parse_sync_bytes(replay_path, file_bytes, apply_hd_early_exit_rules=True, raise_on_error=False):
    try:
        ...
        fallback = _parse_match_fallback_bytes(replay_path, file_bytes, e)
        if fallback:
            if apply_hd_early_exit_rules:
                fallback["parse_reason"] = "de_final_parse_match_fallback"
                fallback.setdefault("key_events", {})["parse_match_final_fallback"] = True
            return fallback
        if raise_on_error:
            raise
        return None
